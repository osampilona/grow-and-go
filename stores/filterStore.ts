...
// Tier 2 (Important Secondary): itemCondition, brands, inStock, onSale
// Tier 3 (Situational): shippingMethods, locationRange, isLocationRangeSet, sellerRating
// Tier 4 (Niche / Misc + Sorting): petFree, smokeFree, perfumeFree, bundleDeal, sortBy
...
const isFiltersActive = (fs: FilterState): boolean => {
  // Order mirrors tier priority for easier scanning
  return (
    (fs.gender?.length ?? 0) > 0 ||
    fs.ageRange[0] !== defaultFilterState.ageRange[0] ||
    fs.ageRange[1] !== defaultFilterState.ageRange[1] ||
    fs.priceRange[0] !== defaultFilterState.priceRange[0] ||
    fs.priceRange[1] !== defaultFilterState.priceRange[1] ||
    (fs.sizes?.length ?? 0) > 0 ||
...
export const useFilterStore = create<FilterStore>()(
  persist(
    (set, get) => ({
      ...
      setTempLocationRange: (locationRange) =>
        set((state) => ({
          tempFilters: {
            ...state.tempFilters,
            locationRange,
            isLocationRangeSet: locationRange > 0,
          },
        })),
      ...
      setTempOnSale: (onSale) =>
        set((state) => ({
          tempFilters: { ...state.tempFilters, onSale },
        })),
      setTempItemCondition: (itemCondition) =>
        set((state) => ({
          tempFilters: { ...state.tempFilters, itemCondition },
        })),
      setTempSellerRating: (sellerRating) =>
        set((state) => ({
      ...
      setTempBrands: (brands) =>
        set((state) => ({
          tempFilters: { ...state.tempFilters, brands },
        })),
      toggleTempBrand: (brand) =>
        set((state) => {
      ...
      toggleTempShippingMethod: (method) =>
        set((state) => {
          ...
          return {
            tempFilters: {
              ...state.tempFilters,
              shippingMethods: current.includes(method)
                ? current.filter((m) => m !== method)
                : [...current, method],
            },
          };
        }),
      setTempBundleDeal: (bundleDeal) =>
        set((state) => ({
          tempFilters: { ...state.tempFilters, bundleDeal },
        })),
      toggleTempBundleDeal: () =>
        set((state) => ({
