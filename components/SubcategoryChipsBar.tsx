...
export const SubcategoryChipsBar = memo(function SubcategoryChipsBar({
  useTemp = false,
  className = "",
  stickyOffset = 64,
}: SubcategoryChipsBarProps) {
  const selectedCategoryId = useCategoryStore((s) => (useTemp ? s.tempSelected[0] : s.selected[0]));
  const toggleSubcategory = useCategoryStore((s) =>
    useTemp ? s.toggleTempSubcategory : s.toggleSubcategory
  );
  const clearFn = useCategoryStore((s) =>
    useTemp ? s.clearTempSubcategoriesForSelected : s.clearSubcategoriesForSelected
  );
  ...
  const selectedSubcategoryIds = useCategoryStore((s) => {
    ...
    return (
      (useTemp ? s.tempSubcategoriesByCategory[catId] : s.subcategoriesByCategory[catId]) ||
      EMPTY_SUBCATEGORY_ARRAY
    );
  });
  ...
  const handleHide = useCallback(() => {
    if (selectedCategoryId && selectedCategoryId !== "everything") {
      useCategoryStore.setState((state) => {
        if (useTemp) {
          const clone = { ...(state.tempSubcategoriesByCategory || {}) } as Record<
            string,
            string[]
          >;
