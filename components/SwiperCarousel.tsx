...
const SwiperCarousel = memo(function SwiperCarousel({
  ...
  onImageClick,
  onReady,
  disableVisibilityOptimization: _disableVisibilityOptimization = false,
    ...
    () => ({
      modules:
        navigationStyle === "arrows" && imagesLength > 1
          ? [EffectCreative, Navigation, Pagination]
          : [EffectCreative],
  ...
  // ResizeObserver to keep Swiper layout in sync; guard against conditional hook usage by always running effect
  useEffect(() => {
    const swiper = swiperRef.current;
    const el = swiper?.el as HTMLElement | null | undefined;
  ...
  // Visibility + bullet repair logic (always register effect, no conditional hook path)
  useEffect(() => {
    const swiper = swiperRef.current;
    const el = swiper?.el as HTMLElement | null | undefined;
      ...
      {aspectRatio && (
        <div
          ...
          style={{
            ...
            width: "100%",
            paddingTop: (() => {
              const p = aspectRatio.split("/");

              if (p.length === 2) {
                const w = parseFloat(p[0]);
